...
def init_store(power_system, times, data):
    ...
    storage.append(
        "power",
        gen_time_dataframe(
            generators, t, values=[[gen.initial_power for gen in generators]]
        ),
        expectedrows=est_Nt,
    )
    ...
    storage.append(
        "status",
        gen_time_dataframe(
            generators, t, values=[[gen.initial_status for gen in generators]]
        ),
        expectedrows=est_Nt,
    )

    storage["hrsinstatus"] = gen_time_dataframe(
        generators, t, values=[[gen.initial_status_hours for gen in generators]]
    )
...
def _get_problem_version():
    version = ""
    possible_init = os.path.join(os.path.abspath(user_config.directory), "__init__.py")
...
def store_state(power_system, times, sln=None):
    ...
    #        status = storage['status'].loc[1:]
    #        power = storage['power'][status.columns].loc[status.index]
    #        cost = storage['observed_cost'][status.columns].loc[status.index]
    #        fuelcost = storage['observed_fuelcost'][status.columns].loc[status.index]
...
def load_state():
    ...
    intervalhrs = (
        times.strings.index[1] - times.strings.index[0]
    ...
    t = times.initialTime
    status = correct_status(storage["status"]).loc[t]
...
def repack_storage():
    ...
    os.system(
        "ptrepack {f} {f}.copy; mv {f}.copy {f};".format(f=user_config.store_filename)
    )
