...
def filter_non_defaults(x, defaults):
    return dict(
        (k, v) for k, v in list(x.items()) if (k not in defaults) or (v != defaults[k])
    )
...
def setup_parser_args(parser):
    parser.add_argument(
        "directory", type=str, help="the direcory of the problem you want to solve"
    )

    add_opt(parser, "solver", "-s", help="the solver name (e.g. cplex, gurobi, glpk)")
    add_opt(parser, "visualization", "-v", help="save a visualization of the solution")
    ...
    add_opt(solver_opt, "mipgap", help="the MIP gap solution tolerence")
    add_opt(
        solver_opt, "solver_time_limit", help="the MIP solver time limit (in seconds)"
    )

    reserve = parser.add_argument_group(
        "Reserve", "Does the system require reserve? The default is no reserve."
    )
    add_opt(
        reserve,
        "reserve_fixed",
    ...
    stochastic = parser.add_argument_group(
        "Stochastic UC", "options to modify the behavior of a stochastic problem"
    )
    add_opt(stochastic, "scenarios", help="limit the number of scenarios to N")
    add_opt(
        stochastic,
        "faststart_resolve",
        "-F",
    ...
    add_opt(parser, "pid", help="Expicitly specify the standalone storage process id")

    debugging = parser.add_argument_group("Debugging tools")
    add_opt(debugging, "debugger", help="use pdb when an error is raised")
    add_opt(
        debugging,
        "problem_file",
        help="flag to write the problem formulation to a problem.lp file",
    )
    add_opt(debugging, "logging_level", help="set the level of detail for logging")
    add_opt(
        debugging,
        "standalone_restart",
    ...
    ts = parser.add_argument_group(
        "Timeseries modifiers", "Alter the timeseries after parsing the data."
    )

    wind_modifier = ts.add_mutually_exclusive_group()
    add_opt(
        wind_modifier, "wind_multiplier", help="scale the wind power by this factor"
    )
    add_opt(
        wind_modifier,
    ...
    add_opt(ts, "load_multiplier", help="scale the load power by this factor")
    add_opt(ts, "load_adder", help="add a fixed amount to the load power")

    constraints = parser.add_argument_group(
        "Ignore/relax constraints",
        "Ignore or relax sets of constraints to allow for feasible solutions.",
    )
    add_opt(
        constraints,
        "ignore_minhours_constraints",
        help="drop the min up/down time constraints on the generators",
    )
    add_opt(
        constraints,
        "ignore_ramping_constraints",
        help="drop the min ramping power constraints on the generators",
    )
    add_opt(
        constraints,
        "ignore_pmin_constraints",
